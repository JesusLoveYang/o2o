...
$(function() {
	...
	$.getJSON(productUrl, function(data) {
		if (data.success) {
			...
			// 商品更新时间
			$('#product-time').text(
					new Date(product.alterTime).Format("yyyy-MM-dd"));
			...
			product.productImgList.map(function(item, index) {
				imgListHtml += '<div> <img src="' + item.productImgAddr
						+ '" width="100%" /></div>';
			});
			// if (data.needQRCode) {
			// // 生成购买商品的二维码供商家扫描
